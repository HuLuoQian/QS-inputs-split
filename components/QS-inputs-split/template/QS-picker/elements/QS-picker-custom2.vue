	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.$nextTick(()=>{
					this.value = QSPicker.getCustomDefaultValue({defaultValue: this.dataSet.defaultValue, linkage: this.linkage, linkageNum: this.linkageNum, itemArrayLen: this.itemArray.length});
					this.checkAsync();
				})
			},
			checkAsync() {
